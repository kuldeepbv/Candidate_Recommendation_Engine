...
# Directly hard-code API key here
    GEMINI_API_KEY = "AIzaSyAB9UyYH9c8oGAYdWESqaxHNt2m5hqeT3k"  
...
class FAISSStore:
    ...
    def add(self, resume_id: str, embedding: np.ndarray, meta: Dict):
        ...
        self.index.add(embedding.astype("float32"))# Store embedding as float32 for compactness
        meta = dict(meta)
...
# Store Embeddings for the resume
def store_resume_embedding(resume_id: str, path_or_bytes: Union[str, Path, bytes, io.BytesIO], filename: Optional[str] = None) -> Dict:
    ...
    mean_emb = np.mean(chunk_embs, axis=0) # normalize mean vector to unit length
